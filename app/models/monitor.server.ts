...
export function monitorLog({
  ...
  type,
  message,
}: Pick<MonitorLogs, 'monitorId' | 'type' | 'message'>) {
  return prisma.monitorLogs.create({
    data: { monitorId, type, message },
  });
}
...
export function driveLog({
  ...
  type,
  message,
}: Pick<DriveLogs, 'driveId' | 'type' | 'message'>) {
  return prisma.driveLogs.create({
    data: { driveId, type, message },
  });
}
...
export function getMonitorLogs({ monitorId }: Pick<MonitorLogs, 'monitorId'>) {
  return prisma.monitorLogs.findMany({
    where: {
      monitorId,
      NOT: {
        message: {
